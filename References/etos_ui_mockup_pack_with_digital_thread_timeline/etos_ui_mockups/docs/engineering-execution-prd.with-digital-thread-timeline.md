...
## Addendum: Interactive Digital Thread Timeline View

The User Experience Layer should include a dedicated Digital Thread Timeline view in addition to the existing Graph Explorer and 360 Context View. This view presents the governed graph as a semantic zoom timeline:

- At macro zoom, the full enterprise digital thread appears as one string-like temporal thread with branches for connected systems.
- At system zoom, users see system endpoints, connection health, live event pulses, branch KPIs, and recent connection events.
- At artifact zoom, users see selected object lineage, relationship labels, event inspector metadata, confidence, data quality, policy status, and evidence links.

The view is read-only in MVP. It renders governed projections from graph memory, import events, identity/data-quality events, document links, recommendations, decisions, audit events, and future ToolRun/AgentRun/WorkflowRun/AgentTeamRun records. Source systems remain system of record, and no enterprise write action is exposed from this timeline.

Required UI behaviors:

- Wheel, pinch, and keyboard zoom with semantic states: macro string, system branch, artifact lineage.
- Drag-to-pan, fit-to-view, minimap, timeline scrubber, live/pause stream, system filters, event type filters, and selected-event inspector.
- All data must be tenant-scoped, permission-filtered, classification-aware, trust-aware, and retrieved through governed backend services.

Proposed implementation module: `DigitalThreadProjectionService`, backed by governed graph/query/context services and a SignalR or SSE live event stream.
