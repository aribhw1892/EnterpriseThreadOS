...
## 5b. Digital Thread Timeline View (Issue 16.1 Addendum / UI Validation)

**Page:** `/digital-thread/timeline`

Validate the semantic zoom timeline against real governed platform data.

Expected results:

- Macro zoom shows the full enterprise thread as one string-like timeline with branches for connected systems.
- Zoom controls, minimap, fit-to-view, time scrubber, live toggle, and filters are visible.
- System branch zoom shows system endpoints such as PDM, ERP, MES, QMS, documents, graph memory, and connector status.
- Event rows are backed by import, graph, identity, data-quality, document, recommendation, and audit data available to the tenant/user.
- Artifact lineage zoom opens a selected event inspector with source system, target system, artifact, relationship, timestamp, confidence, policy status, data quality, graph path, and evidence links.
- Restricted or untrusted data is filtered before rendering; low-trust/conflicted items are shown only as allowed by policy.
- MVP remains read-only: no source-system write action is available from this view.

Optional API sanity checks:

- `GET http://localhost:5000/api/digital-thread/summary`
- `GET http://localhost:5000/api/digital-thread/systems`
- `GET http://localhost:5000/api/digital-thread/events`
- `GET http://localhost:5000/api/digital-thread/branches`
- `GET http://localhost:5000/api/digital-thread/lineage/{artifactId}`
