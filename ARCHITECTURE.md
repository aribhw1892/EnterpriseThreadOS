...
- Decision, outcome, and learning workflows beyond current review-task completion stub. `DecisionArtifact` creation from completed review tasks remains deferred to Issue 20 (`IReviewTaskCompletionHandler` returns `decisionCreationDeferred: true` today). Agent/workflow recommendation creation beyond agent-run path remains partially deferred (`AGENT_DEFERRED` contract for workflow auto-creation).
- Issue 21 Governance Dashboard live KPI analytics and trend charts (Issue 17 provides KPI placeholder catalog only).
- Tool registry, agent execution (`AgentVersion`, `AgentRun`), workflow runtime, multi-agent collaboration, and enterprise action framework. Tool registry, HTTP PydanticAI agent runtime, and governed agent execute APIs are implemented (Issues 22–23); LLM-assisted import mapping preview reuses the same runtime. Workflows, skill runtime composition, and enterprise write actions remain deferred.
