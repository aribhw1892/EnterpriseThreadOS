...
Issues 1–18, the Architectural Abstraction Sprint (Issues 18.1–18.5), Issue 19 (review task artifacts and templates, creation factories from recommendations/data-quality/security/access/manual sources, prerequisite chains with auto-unblock, internal-only assignment, template-gated escalation placeholders, completion with deferred decision hook, governance-flow integration, and minimal `/tasks` UI with debug harnesses), Issue 22 (tool/skill/connector registry with `ToolRun`, dry-run, and disabled write connectors), Issue 23 (tenant `AgentTypeDefinition` and `AgentVersion` artifacts, `AgentRun` runtime records, governed agent execution orchestration via `IToolGateway` and HTTP PydanticAI adapter, preview/test/execute endpoints, recommendation-only output, and AI Trace links), and Issue 24 (closed via 24.1: `WorkflowVersion` artifacts, `WorkflowRun` and `SafeModeEvent` runtime records, `IWorkflowRuntimeAdapter` with `in-process-v1` CI fallback and real `dapr-v1` Dapr Workflow orchestration, inherited risk/trust, manual preview/test/execute, partial safe mode, optimization/business-policy step hooks, recommendation/task-only outputs, and minimal `/workflows` UI) are implemented or partially implemented in the current codebase, including platform foundation, tenant identity/access, governance/audit, artifact registry, classification/policy, graph memory, ontology/model packages, import/mapping/staging with live LLM mapping preview via `pydantic-ai-v1` (`PydanticAiMappingProvider` + agent runtime sidecar, optional `mapping-predictor-tool` prefetch), identity resolution, data quality, documents, governed query/context assembly, AI Trace, governed chat, explorers/360° context views, dashboard/report artifacts, recommendation artifacts with evidence rules, industry-neutral package-driven import/query/recommendation behavior, capability/business-policy/optimization-model/agent-template governed artifacts, mapping provider contracts, agent runtime adapter contracts, the manufacturing reference package under `packages/manufacturing-reference/`, the tool registry with internal `governed-query-v1` and `mapping-predictor-v1` handlers, and tenant agent lifecycle plus execution. Later PRD capabilities such as multi-agent teams, scheduled/event-driven workflow execution, skill runtime composition, and enterprise action framework remain roadmap items unless source code proves otherwise.
