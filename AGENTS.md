...
Issues 1–18, the Architectural Abstraction Sprint (Issues 18.1–18.5), Issue 22 (tool/skill/connector registry with `ToolRun`, dry-run, and disabled write connectors), and Issue 23 (tenant `AgentTypeDefinition` and `AgentVersion` artifacts, `AgentRun` runtime records, governed agent execution orchestration via `IToolGateway` and HTTP PydanticAI adapter, preview/test/execute endpoints, recommendation-only output, and AI Trace links) are implemented or partially implemented in the current codebase, including platform foundation, tenant identity/access, governance/audit, artifact registry, classification/policy, graph memory, ontology/model packages, import/mapping/staging, identity resolution, data quality, documents, governed query/context assembly, AI Trace, governed chat, explorers/360° context views, dashboard/report artifacts, recommendation artifacts with evidence rules, industry-neutral package-driven import/query/recommendation behavior, capability/business-policy/optimization-model/agent-template governed artifacts, mapping provider contracts, agent runtime adapter contracts, the manufacturing reference package under `packages/manufacturing-reference/`, the tool registry with internal `governed-query-v1` handler, and tenant agent lifecycle plus execution. Later PRD capabilities such as review tasks, decisions, outcomes, governance analytics, workflows, and enterprise action framework remain roadmap items unless source code proves otherwise.
