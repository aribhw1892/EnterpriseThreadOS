...
Issues 1–18 and the Architectural Abstraction Sprint (Issues 18.1–18.5) are implemented or partially implemented in the current codebase, including platform foundation, tenant identity/access, governance/audit, artifact registry, classification/policy, graph memory, ontology/model packages, import/mapping/staging, identity resolution, data quality, documents, governed query/context assembly, AI Trace, governed chat, explorers/360° context views, dashboard/report artifacts, recommendation artifacts with evidence rules, industry-neutral package-driven import/query/recommendation behavior, capability/business-policy/optimization-model/agent-template governed artifacts, mapping provider contracts, agent runtime adapter contracts, and the manufacturing reference package under `packages/manufacturing-reference/`. Later PRD capabilities such as review tasks, decisions, outcomes, governance analytics, tool registry, agent execution, workflows, and enterprise action framework remain roadmap items unless source code proves otherwise. Issue 22 is unblocked by 18.5 but not started.
