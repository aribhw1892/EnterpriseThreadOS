...
export default async function ArtifactDetailPage({ params }: { params: Promise<{ artifactId: string }> }) {
  ...
  const [view, flow] = await Promise.all([
    getContextView360("Artifact", artifactId),
    getGovernanceFlow("Artifact", artifactId),
  ]);
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <section className="rounded-3xl border border-slate-800 bg-slate-900 p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-4xl font-semibold">Artifact 360°</h1>
              <p className="mt-3 font-mono text-sm text-cyan-200">{artifactId}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <ExplorerNavLink href="/artifacts">Artifacts</ExplorerNavLink>
