...
import type { PdmImportFileProfile, PdmImportManifest } from "@/lib/pdm-import-types";

type PdmTransformUploadProps = {
  profiles: PdmImportFileProfile[];
  manifest?: PdmImportManifest | null;
};

export function PdmTransformUpload({ profiles, manifest }: PdmTransformUploadProps) {
  return (
    <div className="grid gap-4">
      {manifest?.outputs ? (
        <div className="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 p-4 text-sm text-slate-200">
